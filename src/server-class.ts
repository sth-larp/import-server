...
import { JoinImporter, JoinCharacter, JoinCharacterDetail } from "./join-importer";
...
export class Server<Model extends AliceBaseModel> {
    ...
    private workData : ModelImportData = new ModelImportData();
 ...
 */
prepareForImport(data: ModelImportData): Observable<ModelImportData> {
 ...
 */
loadCharacterListFomJoin(data: ModelImportData): Observable<ModelImportData> {
 ...
 */
loadCharacterListFromCache(data: ModelImportData): Observable<ModelImportData> {
    return Observable.fromPromise(
 ...
 */
saveCharacterToCache(char: JoinCharacterDetail, data: ModelImportData): Observable<JoinCharacterDetail> {
...
perfromProvide (
    ...
    if (this.params.ignoreInGame || !char.finalInGame) {
...
exportCharacterModel(
    ...
    const exporter = new AliceExporter(this.facade, char, data.importer.metadata, true, this.params.ignoreInGame);
 ...
 */
sendModelRefresh(char: JoinCharacterDetail, data: ModelImportData): Observable<JoinCharacterDetail> {
 ...
 */
loadCharactersFromJoin(data: ModelImportData): Observable<JoinCharacterDetail> {
    let bufferCounter = 0;
 ...
 */
loadCharactersFromCache(data: ModelImportData): Observable<JoinCharacterDetail> {
    let bufferCounter = 0;
...
importAndCreate(   
    ...
    // Загрузить данные из Join или из кеша
        .flatMap( (data: ModelImportData) => importJoin ? this.loadCharactersFromJoin(data) : this.loadCharactersFromCache(data) )
    ...
    // Экспортировать модель в БД (если надо)
        ...
        .flatMap( (c)  => this.exportCharacterModel(c, this.workData, exportModel) )
    ...
    // Выполнить все задачи после экспорта
        this.facade.getAfterConversionProviders().forEach(provider => {
            chain = chain.flatMap((c) => this.perfromProvide(provider, c, this.workData, exportModel))
    ...
    // Послать модели Refresh соообщение для создания Work и View-моделей
        .flatMap( (c) => refreshModel ? this.sendModelRefresh(c, this.workData) : Observable.from([c]) )
